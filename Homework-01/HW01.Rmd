...
---
1. **Solve**. 
    a. Noticing that the data in iowa.csv is seperated by ";", I choose to add an argument `sep = ";"` to the funtion `read.csv()`.
    ```{r}
...
2. **Solve**. 
    a. The first line creates a vector of characters(or strings) named `vactor1`.
    ```{r}
    vector1 <- c("5", "12", "7", "32")
    ```
...
3. **Solve**.
    a. Use arguments `by` and `length.out` in `seq()` to achieve the target.
    ```{r}
    seq(1, 10000, by = 372)
...
5. (MB.CH1.4) **Solve**. 
    a. There's no column holds missing values.
    ```{r}
    data(ais)
    str(ais)
