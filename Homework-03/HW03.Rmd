...
1. 
    ...
    gmp %>% ggplot + geom_point(aes(x = pop, y = pcgmp)) +
      ...
      geom_line(aes(x = pop, y = a0.1), col = 'red', size = 1.5) +
      geom_line(aes(x = pop, y = a0.15), col = 'green', size = 1.5) +
      scale_x_log10()
```
...
7. 
    ```{r}
    plm.jackknife <- function(y0, a){
      jackknifed.gmp <- data.frame()
      for (i in 1:nrow(gmp)){
        m = data.frame(plm(y0, a, N = gmp[-i,"pop"], Y=gmp[-i,"pcgmp"]))
